...
namespace CardSolutionHost.Core
{
    ...
    public class ClassValueCopier
    {
        ...
        public static List<D> GetArrayFromDataTable<D>(DataTable source)
        {
            ...
            {
                var dr = source.Rows[i];
                D t = (D)Activator.CreateInstance(typeof(D));
                CopyValueFromDataRow(t, dr);
                results.Add(t);
            }
            return results;
        }

        public static void CopyValueFromDataRow<D>(D destination, DataRow source)
        {
            if (destination == null || source == null)
                return;
            var ps = typeof(D).GetProperties();
            var dt = source.Table;
            foreach (var p in ps)
            {
                if (dt.Columns.Contains(p.Name))
                    p.SetValue(destination, source[p.Name], null);
            }
        }
